...
import com.dove.sprout.common.exception.BaseException;
...
public class VerificationUtil {
	...
	public static void isNotBlank(String... strs) {
		if(strs==null||strs.length==0){
			throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.STRING_NOT_NULL);
		}
		for(String str:strs){
	...
	public static void notNull(Object... objects) {
		if(objects==null||objects.length==0){
			throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.OBJECT_NOT_NULL);
		}
	...
	public static void notEmpty(Map map) {
		if (map == null || map.isEmpty()) {
			throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.MAP_NOT_NULL);
		}
	}
	...
	public static void notNullNodes(Collection collection) {
		...
		for (Iterator it = collection.iterator(); it.hasNext();) {
			if (it.next() == null) {
				throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.COLLECTION_NOT_NULL);
			}
		}
	}
	...
	public static void isTrue(boolean... bos) {
		if(bos==null||bos.length==0){
			throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.BOOLEAN_NOT_FALSE);
		}
		for(boolean bo:bos){
			if (!bo) {
				throw new ArgumentNotValidException(ArgumentNotValidException.ARGUMENT_VALI_EXP.BOOLEAN_NOT_FALSE);
